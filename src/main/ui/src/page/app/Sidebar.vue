<template>
  <el-menu class="sidebar" style="min-height: 100%" mode="vertical" :unique-opened="true" :default-active="$route.path" :collapse="!sidebarOpen" background-color="#304156" text-color="#fff" active-text-color="#409EFF">
    <el-submenu v-if="menu.show" v-for="menu in menus" :index="menu.path" :key="menu.path">
      <template slot="title">
        <i :class="`el-icon-${menu.iconName}`"></i>
        <span v-text="menu.name"></span>
      </template>
      <el-menu-item v-if="menuItem.show" v-for="menuItem in menu.subMenus" :index="`${menu.path}${menuItem.path}`" :key="`${menu.path}${menuItem.path}`">
