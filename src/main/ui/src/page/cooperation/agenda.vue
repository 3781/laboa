  ...
  <div class="agenda-container">
    ...
    <el-table :data="agendas" v-loading="loading" size="small" :stripe="true" :border="true" ref="agendaTable" @sort-change="handleSortChange"
      ...
      <el-table-column align="center" label="单位" column-key="units" prop="unit"
                       sortable="custom" :resizable="true" filter-placement="top" width="80px">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(['listCooperationAgendas', 'closeAgenda', 'openAgenda', 'deleteAgenda', 'updateAgenda']),
      ...
      getAgendasData() {
        ...
        if (this.cooperationId != null) {
          this.listCooperationAgendas({
            cooperationId: this.cooperationId,
            formData: this.agendaSelectQuery,
          }).then((response) => {
